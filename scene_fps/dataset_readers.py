...
from scene_fps.colmap_loader import read_extrinsics_text, read_intrinsics_text, qvec2rotmat,\
...
from scene_fps.gaussian_model import BasicPointCloud
import random
try:
    import laspy
except:
...
def readColmapSceneInfo_colmap(path, images, eval, llffhold=7):
    try:
...
def readColmapSceneInfo(path, images, eval, llffhold=7):
    try:
        cameras_extrinsic_file = os.path.join(path, "sparse_vggt", "images.bin")
        cameras_intrinsic_file = os.path.join(path, "sparse_vggt", "cameras.bin")
        cam_extrinsics = read_extrinsics_binary(cameras_extrinsic_file)
        cam_intrinsics = read_intrinsics_binary(cameras_intrinsic_file)
    except:
        cameras_extrinsic_file = os.path.join(path, "sparse_vggt", "images.txt")
        cameras_intrinsic_file = os.path.join(path, "sparse_vggt", "cameras.txt")
        cam_extrinsics = read_extrinsics_text(cameras_extrinsic_file)
        cam_intrinsics = read_intrinsics_text(cameras_intrinsic_file)

    reading_dir = "images_vggt" #if images == None else images
    ...
    ply_path = os.path.join(path, "sparse_vggt/points3D.ply")
    bin_path = os.path.join(path, "sparse_vggt/points3D.bin")
    txt_path = os.path.join(path, "sparse_vggt/points3D.txt")
    if not os.path.exists(ply_path):
        print("Converting point3d.bin to .ply, will happen only the first time you open the scene.")
        try:
            xyz, rgb, _ = read_points3D_binary(bin_path)
...
sceneLoadTypeCallbacks = {
    ...
    "Colmap_queen": readColmapSceneInfo_colmap,
