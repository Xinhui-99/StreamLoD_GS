...
from scene_fps.decoders import LatentDecoder, DecoderIdentity, DecoderLayer, LatentDecoderRes, Gate
import time
from scene_fps.embedding import Embedding
...
class GaussianModel:
    ...
    def add_anchor(self):
        ...
        cur_size = self.voxel_size / (float(self.fork) ** 1)

        fused_anchor = torch.tensor(np.asarray(new_anchor0.cpu())).float().cuda()
        ...
        init = torch.zeros((new_anchor0.size(0), self.latent_decoders["f_feature_anchor"].latent_dim)).to(init_anchor_feat).contiguous()
    ...
    def anchor_grad_2frame(self, xyz_save):
        ...
        all_values = grad_dist * (self.get_level_before == torch.round(torch.tensor(self.levels / 2.0))).squeeze() 
        ...
        threshold = np.percentile(labels, 75)  # 获取前15%的阈值，85%的位置代表前15%
        # 筛选出大于该阈值的点
