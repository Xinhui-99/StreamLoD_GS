...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int,
        ...
        self.lod_scaling = torch.empty(0)
        ...
        # 添加静态MLP存储（第一帧后会被初始化）
        self.mlp_opacity_static = None
        self.mlp_cov_static = None  
        self.mlp_color_static = None
        self.mlp_feature_bank_static = None if not self.use_feat_bank else None
        ...
        self.mlp_color = nn.Sequential(
                ...
                nn.ReLU(True),
                nn.Linear(self.feat_dim, (3 * ((sh_degree + 1) ** 2 ))*self.n_offsets),
                nn.Sigmoid()
            ).cuda()

        self.mlp_opacity_static = nn.Sequential(
                nn.Linear(self.feat_dim+self.view_dim+self.opacity_dist_dim+self.level_dim, self.feat_dim),
                nn.ReLU(True),
                nn.Linear(self.feat_dim, self.n_offsets),
                nn.Tanh()
        ...
        self.mlp_cov_static = nn.Sequential(
                nn.Linear(self.feat_dim+self.view_dim+self.cov_dist_dim+self.level_dim, self.feat_dim),
                nn.ReLU(True),
                nn.Linear(self.feat_dim, 7*self.n_offsets),
            ).cuda()
        
        self.mlp_color_static = nn.Sequential(
                nn.Linear(self.feat_dim+self.view_dim+self.color_dist_dim+self.level_dim+self.appearance_dim, self.feat_dim),
                nn.ReLU(True),
                nn.Linear(self.feat_dim, (3 * ((sh_degree + 1) ** 2 ))*self.n_offsets),
                nn.Sigmoid()
    ...
    def _features_anchor(self):
        ...
        if isinstance(self.latent_decoders["f_feature_anchor"], DecoderIdentity):
            feature_anchor = self._latents["f_feature_anchor"]
        else:
            feature_anchor = self.latent_decoders["f_feature_anchor"](self._latents["f_feature_anchor"])
            feature_anchor = feature_anchor.reshape(feature_anchor.shape[0], 1, 3)
        
        # Apply gating if enabled
        if self.prev_atts["f_feature_anchor"] is not None and self.gate_atts is not None and self.gate_params["f_feature_anchor"]:
            feature_anchor = self.gate_atts(feature_anchor-self.prev_atts["f_feature_anchor"])+self.prev_atts["f_feature_anchor"]
        return feature_anchor  # shape (N, 1, 3)
    ...
    def get_scaling(self):
        ...
        return self.scaling_activation(self.lod_scaling)
 
    @property
    def get_rotation(self):
        return self.rotation_activation(self.lod_rotation)
    ...
    def generate_neural_gaussians_save(self):
        ...
        self._offset_before = self._offset.data
        self.get_scaling_before = self.get_scaling.data
        self.get_rotation_before = self.get_rotation.data
        ...
        self.lod_scaling_before = self.lod_scaling.data.clone()  # log空间
        ...
        # 2. 最快的MLP复制方式
        for name in ['opacity', 'cov', 'color']:
            source = getattr(self, f'mlp_{name}')
            ...
            setattr(self, f'mlp_{name}_static', target)
    ...
    def generate_neural_gaussians_prev(self, viewpoint_camera, visible_mask=None, is_train=None, grad=False):
        ...
        def forward(self, viewpoint_camera, visible_mask=None, is_train=None, grad=False):
            ...
            #level = self.get_level_before[visible_mask]
            grid_offsets = self._offset_before[visible_mask]
            ...
            with torch.no_grad():
                ...
                neural_opacity = self.mlp_opacity_static(cat_local_view_wodist)
                color = self.mlp_color_static(cat_local_view_wodist)
                scale_rot = self.mlp_cov_static(cat_local_view_wodist)
    ...
    def create_from_pcd(self, pcd, point_cloud: BasicPointCloud, spatial_lr_scale : float, ignore_colors: bool = False):
        ...
        self.lod_scaling = nn.Parameter(scales.requires_grad_(True))
        ...
        self._anchor_mask_grad = torch.zeros(self._anchor.shape[0], dtype=torch.bool, device="cuda")
        self._anchor_mask_grad_report = torch.ones(self._anchor.shape[0], dtype=torch.bool, device="cuda")
        #fused_point_cloud = torch.tensor(np.asarray(point_cloud.points)).float().cuda()
        
        if ignore_colors:
            fused_color = RGB2SH(torch.tensor(0.5 * np.ones_like(point_cloud.points)).float().cuda())
        else:
        ...
        init = self.latent_decoders["anchor"].invert(self._anchor)
    ...
    def size(self):
        ...
        with torch.no_grad():
            ...
            ldec_size += self.size_mlp(self.mlp_opacity) * 2
            ldec_size += self.size_mlp(self.mlp_cov) * 2
            ldec_size += self.size_mlp(self.mlp_color) * 2

            if self.appearance_dim > 0:
                ldec_size += self.embedding_appearance.size()

            # 原始大小（未压缩）
            anchor_size_raw = self._anchor.numel() * torch.finfo(self._anchor.dtype).bits
            anchor_feat_size_raw = self._anchor_feat.numel() * torch.finfo(self._anchor_feat.dtype).bits
            scaling_size_raw = self.lod_scaling.numel() * torch.finfo(self.lod_scaling.dtype).bits
            level_size_raw = self._level.numel() * self._level.element_size() * 8  # element_size() 返回字节数，需要转换为位数
            
            # 压缩率估计（根据经验或实验确定）
            # 一般神经网络参数可以压缩到 20%-50% 的原始大小
            compression_ratio = 0.3  # 30% 的原始大小（可调整）
            
            latents_size = anchor_size_raw + (anchor_feat_size_raw + scaling_size_raw + level_size_raw) * compression_ratio
    ...
    def training_setup(self, training_args):
        ...
        self.anchor_gradient_accum = torch.zeros((self._anchor.shape[0], 1), device="cuda")
        ...
        self.denom_lod = torch.zeros((self._anchor.shape[0], 1), device="cuda")
        ...
        self.offset_gradient_accum_lod = torch.zeros((self.get_anchor.shape[0]*self.n_offsets, 1), device="cuda")
        self.offset_denom = torch.zeros((self.get_anchor.shape[0]*self.n_offsets, 1), device="cuda")
        self.offset_denom_lod = torch.zeros((self.get_anchor.shape[0]*self.n_offsets, 1), device="cuda")
        self.anchor_demon = torch.zeros((self.get_anchor.shape[0], 1), device="cuda")
        
        l = [
            {'params': [self._anchor], 'lr': training_args.position_lr_init * self.spatial_lr_scale, "name": "anchor"},
            {'params': [self._offset], 'lr': training_args.offset_lr_init * self.spatial_lr_scale, "name": "offset"},
            {'params': [self._anchor_feat], 'lr': training_args.feature_lr, "name": "f_feature_anchor"},
            {'params': [self.lod_opacity], 'lr': training_args.opacity_lr, "name": "opacity"},
            {'params': [self.lod_scaling], 'lr': training_args.scaling_lr, "name": "scaling"},
    ...
    def construct_list_of_attributes_lod(self):
        ...
        l.append('opacity')
        for i in range(self.lod_scaling.shape[1]):
    ...
    def save_ply_lod(self, path):
        ...
        scales = self.lod_scaling.detach().cpu().numpy()
        ...
        for i,attribute in enumerate(self.param_names):
            if isinstance(self.latent_decoders[attribute], DecoderIdentity):
                if self.prev_atts[attribute] is not None and self.gate_atts is not None and self.gate_params[attribute]:
                    anchor = self.latent_decoders["anchor"](self._latents["anchor"])
                    prev = self.anchor_before[self.mapping]
                    residual_anchor = anchor - prev
                    
                    # Get ungated indices and their residual values
                    ungated_anchor_indices = self.gate_atts.sample_gate(stochastic=False).nonzero(as_tuple=True)[0]
                    # compute the number of bits needed to store the max value in ungated_anchor_indices
                    max_value = ungated_anchor_indices.max()
                    num_bits = max_value.item().bit_length()
                    if num_bits < 8:
                        ungated_anchor_indices = ungated_anchor_indices.type(torch.int8)
                    elif num_bits < 16:
                        ungated_anchor_indices = ungated_anchor_indices.type(torch.short)
                    elif num_bits < 32:
                        ungated_anchor_indices = ungated_anchor_indices.type(torch.int)
                    else:
                        ungated_anchor_indices = ungated_anchor_indices.type(torch.long)
                    # Store the gated residuals, not raw residuals
                    ungated_residuals = self.gate_atts(residual_anchor)[ungated_anchor_indices]
                    ...
                    latents[attribute] = {
                        ...
                        'ungated_indices': ungated_anchor_indices,
                        'ungated_residuals_compressed': compressed_residuals
                    }
                    
                    # Verify reconstruction matches _anchor
                    reconstructed_anchor = prev.clone()
                    reconstructed_anchor[ungated_anchor_indices] += ungated_residuals
                    # assert torch.allclose(self._anchor, reconstructed_anchor, rtol=1e-5, atol=1e-5), "Reconstruction verification failed!"
                else:
        ...
        with torch.no_grad():
            for param in self.param_names:
                if not isinstance(self.latent_decoders[param], DecoderIdentity):
                    ...
                    if param == "f_feature_anchor":
                        self._latents[param].data = decoded.reshape(self._latents[param].shape[0], 32, 3)
                    else:
    ...
    def add_anchor(self):
        new_anchor = self.get_anchor_before[self.diff_mask]
        new_feat = self.get_anchor_feat_before[self.diff_mask]
        new_level = self.get_level_before[self.diff_mask]
        new_offsets = self._offset_before[self.diff_mask]

        # 关键修改:使用原始空间的参数
        new_scaling = self.lod_scaling_before[self.diff_mask]  # 已经在log空间
        ...
        self.lod_scaling = optimizable_tensors["scaling"]
        self.lod_rotation = optimizable_tensors["rotation"]
        self._anchor_feat = optimizable_tensors["f_feature_anchor"]
    ...
    def refine_anchor(self):
        ...
        # 注意：_prune_anchor_optimizer 接受的是"保留"的mask
        optimizable_tensors = self._prune_anchor_optimizer(self._anchor_mask_grad)
    ...
    def anchor_grad_2frame(self, xyz_save):
        ...
        # 获取当前level的梯度和坐标
        level_xyz = xyz_save.detach().cpu().numpy()  # [N, 3]

        # 提取正梯度的坐标和梯度     
        pos_xyz = level_xyz[pos_mask]  # [M, 3]
        
        # 归一化坐标
        scaler_xyz = StandardScaler()
        norm_xyz = scaler_xyz.fit_transform(pos_xyz)        
        # 组合特征 [坐标(3维) + 梯度(1维)] -> [M, 4]
        # 可以调整权重来平衡坐标和梯度的重要性
        grad_weight = 1.0  # 梯度权重（可调）
        features = np.concatenate([norm_xyz, norm_grads * grad_weight], axis=1)
    ...
    def anchor_growing(self, iteration, grads, threshold, update_ratio, extra_ratio, extra_up, offset_mask,frame_idx=1):
        ...
        for cur_level in range(self.levels):
            ...
            if (iteration > self.coarse_intervals[-1]) and cur_level < self.levels - 1:
